...
export function EventItem({ event, result }: EventItemProps) {
  ...
  switch (event.type) {
    ...
    case 'tool-use':
      ...
      return (
        <div className="my-3 rounded-lg border border-blue-500/40 bg-blue-950/40 overflow-hidden">
          {/* Tool Call Header */}
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="w-full flex items-center gap-2 px-3 py-2.5 text-left hover:bg-blue-900/30 transition-colors"
          >
            {isExpanded ? (
            ...
            <Wrench size={14} className="text-blue-400 flex-shrink-0" />
            <span className="font-semibold text-blue-300 text-sm">
              {event.name}
            </span>
            {!isExpanded && inputPreview && (
              <span className="text-xs text-blue-400/60 truncate ml-2 flex-1">
                {inputPreview}
              </span>
            )}
            {/* Status indicator */}
            {hasResult ? (
              ...
              <Loader2 size={14} className="text-blue-400 flex-shrink-0 ml-auto animate-spin" />
            )}
          </button>

          {/* Expanded Input */}
          {isExpanded && (
            <div className="px-3 pb-3 border-t border-blue-500/20 bg-blue-950/30">
              <pre className="mt-2 text-xs text-blue-200 overflow-x-auto whitespace-pre-wrap font-mono p-3 rounded bg-black/30 max-h-64 overflow-y-auto">
          ...
          {toolResult && (
            <div className="border-t border-green-500/30 bg-green-950/30">
              <button
                onClick={() => setIsResultExpanded(!isResultExpanded)}
                className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-green-900/20 transition-colors"
              >
                {isResultExpanded ? (
                  <ChevronDown size={12} className="text-green-400 flex-shrink-0" />
                ) : (
                  <ChevronRight size={12} className="text-green-400 flex-shrink-0" />
                )}
                <CheckCircle size={12} className="text-green-400 flex-shrink-0" />
                <span className="font-medium text-green-300 text-xs">Result</span>
              </button>
              <div className="px-3 pb-3">
                {isResultExpanded ? (
                  <pre className="text-xs text-green-200 overflow-x-auto whitespace-pre-wrap font-mono p-3 rounded bg-black/30 max-h-96 overflow-y-auto">
                    {toolResultText}
                  </pre>
                ) : (
                  <p className="text-xs text-green-300/80 whitespace-pre-wrap line-clamp-3">
                    {resultPreviewTool}
                  </p>
                )}
              </div>
    ...
    case 'tool-result':
      ...
      return (
        <div className="my-3 rounded-lg border border-green-500/40 bg-green-950/40 overflow-hidden">
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="w-full flex items-center gap-2 px-3 py-2.5 text-left hover:bg-green-900/30 transition-colors"
          >
            {isExpanded ? (
          ...
          <div className="px-3 pb-3 border-t border-green-500/20 bg-green-950/30">
            {isExpanded ? (
              <pre className="mt-2 text-xs text-green-200 overflow-x-auto whitespace-pre-wrap font-mono p-3 rounded bg-black/30 max-h-96 overflow-y-auto">
                {standaloneResultText}
              </pre>
            ) : (
              <p className="mt-2 text-xs text-green-300/80 whitespace-pre-wrap line-clamp-3">
    ...
    case 'thinking':
      ...
      return (
        <span className="text-gray-400 italic prose prose-invert prose-sm max-w-none">
    ...
    case 'todo-list': {
      ...
      return (
        <div className="my-3 rounded-lg border border-violet-500/40 bg-violet-950/40 overflow-hidden">
          {/* Header with progress */}
          <div className="flex items-center gap-2 px-3 py-2.5 border-b border-violet-500/20">
            <ListTodo size={14} className="text-violet-400 flex-shrink-0" />
            <span className="font-semibold text-violet-300 text-sm">Agent Tasks</span>
            <span className="text-xs text-violet-400/70 ml-auto">
              {completedCount}/{totalCount}
            </span>
          </div>

          {/* Progress bar */}
          <div className="h-1 bg-violet-950">
            <div
            ...
            {items.map((item, idx) => (
              <div
                ...
                className={`flex items-start gap-2 text-sm ${
                  item.completed ? 'text-violet-400/60' : 'text-violet-200'
                }`}
              >
                {item.completed ? (
                  <CheckCircle2 size={14} className="text-violet-400 flex-shrink-0 mt-0.5" />
                ) : (
                  <Circle size={14} className="text-violet-500/50 flex-shrink-0 mt-0.5" />
                )}
    ...
    case 'complete':
      ...
      const completePreview = completeText.length > 200
        ...
        : completeText;
      return (
        <div className="my-3 p-4 rounded-lg border border-green-500/40 bg-green-950/40">
          <div className="flex items-center gap-2 text-green-300 mb-2">
            <CheckCircle size={16} />
            <span className="font-semibold">Completed</span>
          </div>
          <p className="text-sm text-green-200/90 whitespace-pre-wrap leading-relaxed">
            {completePreview}
          </p>
        </div>
      );
    ...
    case 'error':
      return (
        <div className="my-3 p-4 rounded-lg border border-red-500/40 bg-red-950/40">
    ...
    case 'run-start':
      return (
        <div className="my-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-cyan-950/60 border border-cyan-500/40">
